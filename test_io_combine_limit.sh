...
LOGS_ROOT="${LOGS_ROOT:-logs/io_combine_limit}"
DRYRUN="${DRYRUN:-0}"
...
cleanup(){ [ "$DRYRUN" = 1 ] && return; echo; echo "==> resetting all tuned parameters to default"; EXTRA_PARAMS="$SWEPT_GUCS" bash "$HERE/reset_all_parameters.sh" $PGVERS; }
...
for ver in $PGVERS; do
    ...
    if ! pg_ctlcluster "$ver" main restart 2>/dev/null; then
        ...
        EXTRA_PARAMS="$SWEPT_GUCS" bash "$HERE/reset_all_parameters.sh" "$ver"
        continue
    fi
