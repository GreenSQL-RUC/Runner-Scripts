-- StackOverflow schema for the SQLStorm v1.0 benchmark, WITHOUT foreign keys.
-- Verbatim copy of v1.0/stackoverflow/schema_nofk.sql from
--   https://github.com/SQL-Storm/SQLStorm @ b3bb0b96794a6afe9bb8f3ff2b243562b779c40d
-- The no-FK variant is the one this benchmark loads: tables can be loaded in
-- any order and no FK checks run during the load. PostgreSQL also uses FKs for
-- join-size estimates, so plans on a schema.sql (with FK) build can differ.
-- Used by build/build_stackoverflow.sh.
