 ...
 * `make cold` handles the sudo. ALL cold runs go through `make cold`; the warm
 * runner (`make run`) never drops caches or restarts anything.
 *
...
int main(void) {
    ...
    /* The cold CSV is <LOGS_DIR>/query_cold_<db>.csv; create the dir so the
     * append does not fail. */
    const char *logs_dir = env_or("LOGS_DIR", DEFAULT_LOGS_DIR);
    (void)mkdir(logs_dir, 0755);

    char cold_log[PATH_MAX];
    snprintf(cold_log, sizeof(cold_log), "%s/%s%s.csv", logs_dir, DEFAULT_COLD_PREFIX, db_name);
