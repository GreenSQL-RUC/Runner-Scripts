...
/* The *_ENERGY_STATUS MSRs are 32-bit counters that wrap every 2^32 energy
 * units (about 262144 J at the common 61 uJ unit, i.e. every couple of hours
 * at laptop power). A delta that went negative crossed the wrap: add one full
 * counter span back. */
static double rapl_wrap(double delta) {
  if (delta < 0) delta += 4294967296.0 * energy_units;
  return delta;
}
...
{ int fd;
  ...
  package_delta = rapl_wrap(package_after - package_before);

  result=read_msr(fd,MSR_PP0_ENERGY_STATUS);
  pp0_after=(double)result*energy_units;
  pp0_delta = rapl_wrap(pp0_after - pp0_before);
  ...
  (cpu_model==CPU_HASWELL)) {
     ...
     has_pp1 = 1;
     pp1_delta = rapl_wrap(pp1_after - pp1_before);
  }
  ...
  (cpu_model==CPU_HASWELL)) {
     ...
     has_dram = 1;
     dram_delta = rapl_wrap(dram_after - dram_before);
  }
