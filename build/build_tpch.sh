...
#
# SKEWED DATA. SKEW=<z> generates with Microsoft Research's Zipfian TPC-D dbgen
# modification (Linux port, fetched on demand into tpch-dbgen-skew/ like the
# stock dbgen) instead of the stock generator: every column is drawn from a
# Zipfian distribution with parameter z (0 = uniform, 4 = extreme; -1 = a random
# z per column from {0..4}). Same tables, columns and file format, so the load
# is identical. NOTE: the skew tool is the older TPC-D 1.3.1 code base, whose
# comment strings and lineitem row count differ slightly from the stock 2.x
# dbgen, so a skewed/unskewed COMPARISON should generate both sides with it:
#   sudo SKEW=0 bash build_tpch.sh 1 tpch_z0 18     # the comparable uniform baseline
#   sudo SKEW=2 bash build_tpch.sh 1 tpch_z2 18     # the skewed twin
# GEN_ONLY=1 stops after generating the .tbl files (nothing touches PostgreSQL).
#
set -euo pipefail
...
SCHEMA="$ROOT/schema/tpch_schema.sql"
PGUSER=postgres
...
# The skewed generator (see header). Its upstream Makefile is broken ("-o @"),
# so it is compiled with a direct gcc line below. Pinned like the stock one.
SKEW="${SKEW:-}"
SKEW_REPO="${SKEW_REPO:-https://github.com/gunaprsd/SkewedDataGenerator}"
SKEW_COMMIT="${SKEW_COMMIT:-28a9c7045d317075c3395303c739d7953c8168b6}"
if [ -n "$SKEW" ]; then
    DBGEN_DIR="$ROOT/tpch-dbgen-skew"
    DBGEN_REPO="$SKEW_REPO"; DBGEN_COMMIT="$SKEW_COMMIT"
fi
...
if [ "${SKIP_DBGEN:-0}" = "1" ] && [ -s "$DBGEN_DIR/lineitem.tbl" ]; then
    ...
    if [ ! -x "$DBGEN_DIR/dbgen" ]; then
        ...
        if [ -n "$SKEW" ]; then
            ( cd "$DBGEN_DIR" && gcc *.c -o dbgen -lm -w )
        else
            ensure_dbgen_makefile
    ...
    if [ -n "$SKEW" ]; then
        echo "==> [$DB] generating SKEWED TPC-H data at scale factor $SF, zipf z=$SKEW (this can take a while)"
        ( cd "$DBGEN_DIR" && ./dbgen -f -s "$SF" -z "$SKEW" )
        # The TPC-D era generator names the orders file order.tbl.
        [ -f "$DBGEN_DIR/order.tbl" ] && mv -f "$DBGEN_DIR/order.tbl" "$DBGEN_DIR/orders.tbl"
    else
...
if [ "${GEN_ONLY:-0}" = "1" ]; then
    echo "==> [$DB] GEN_ONLY=1: .tbl files are in $DBGEN_DIR (not loaded, not removed):"
    wc -l "$DBGEN_DIR"/*.tbl
    exit 0
fi
