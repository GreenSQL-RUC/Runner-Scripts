...
to queries/slow/tpch/tpch-queries/ instead, so a normal sweep of queries/tpch
...
Usage:
  python3 build/generate_tpch_query_set.py              # (re)generate the tree
  python3 build/generate_tpch_query_set.py --check      # + validate on `tpch`:
...
OUT_ROOT = os.path.join(REPO, "queries", "tpch", "tpch-queries")
# Pathologically slow files go here instead, so a normal sweep does not stall on
# them (run them on their own with `make run DIR=queries/slow/tpch/tpch-queries`).
SLOW_ROOT = os.path.join(REPO, "queries", "slow", "tpch", "tpch-queries")
