#!/usr/bin/env python3
"""
Generate the WRITE (data-mutating) benchmark corpus under write_queries/tpch/.
...
Run:  python3 write_queries/generate_tpch_write_queries.py
"""
import os
import shutil

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tpch")   # write_queries/tpch/
...
def render(section_title, name, note, setup, measure):
    ...
    for ln in note.split("\n"):
        lines.append("-- " + ln)
    lines.append("-- Isolated write: runs only on the scratch DB (tpch_write). Everything up to")
    lines.append("-- @MEASURE is SETUP - it rebuilds a disposable w_* table from read-only")
    lines.append("-- reference data and is NOT timed; only the statement after @MEASURE is measured.")
