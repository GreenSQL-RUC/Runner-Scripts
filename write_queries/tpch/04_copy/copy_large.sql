...
-- Operation: copy_large
-- Bulk load of 1000000 rows via server-side COPY FROM (the fast load path).
...
COPY (SELECT * FROM lineitem LIMIT 1000000) TO '/tmp/w_copy_large.csv' WITH (FORMAT csv);
-- @MEASURE
COPY w_lineitem FROM '/tmp/w_copy_large.csv' WITH (FORMAT csv);
