...
-- per outer row.
-- 150k executions of a 25-row seq scan: per-invocation SubPlan cost.
...
SELECT (SELECT n_name FROM nation WHERE n_nationkey = c_nationkey) FROM customer;
