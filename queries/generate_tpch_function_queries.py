...
Usage:  python3 queries/generate_tpch_function_queries.py
...
OUT_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tpch", "Functions")
...
# them and they can be run on their own: `make run DIR=slow_queries`. Add
...
LONG_ROOT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "slow_queries", "tpch")
LONG_RUNNING = {
...
def main():
    ...
    for section_dir, title, note, ops in SPEC:
        for op in ops:
            ...
            # Route pathologically slow operations to the separate slow_queries
    ...
    print("%-26s %3d queries -> slow_queries" % ("long-running", long_total))
