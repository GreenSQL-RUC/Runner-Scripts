...
-- Operation: copy_medium
-- Bulk load of 100000 rows via server-side COPY FROM (the fast load path).
...
COPY (SELECT * FROM lineitem LIMIT 100000) TO '/tmp/w_copy_medium.csv' WITH (FORMAT csv);
-- @MEASURE
COPY w_lineitem FROM '/tmp/w_copy_medium.csv' WITH (FORMAT csv);
